...
def evaluate_grid_point(
    task: tuple[int, int, float, float, np.ndarray, int, float],
) -> tuple[int, int, float, float]:
    idx_y, idx_x, x, y, anchors, num_trials, transmitter_z = task
...
def main() -> None:
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        futures = [executor.submit(evaluate_grid_point, task) for task in tasks]
    ...
    plt.title(
        ...
        f"Transmitter Height: 0.5m | Trials: {args.num_trials}",
        fontsize=13,
    )
    plt.xlabel("X (meters)")
    plt.ylabel("Y (meters)")
